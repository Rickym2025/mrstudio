...
export function NovaChatbot() {
  ...
  // Sequenza dei tuoi loghi presenti nella cartella public
  const logos = [
    "/logo.png",
  ...
  // Gestione del timer per scorrere i loghi in sequenza ogni 3 secondi
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-24 md:bottom-8 left-4 md:left-8 z-[100] flex flex-col items-start">
      ...
      {/* Finestra principale con bordo multicolore animato incorporato */}
      <div className={`bg-[#0a0a0f]/95 backdrop-blur-2xl animated-gradient-border rounded-2xl w-[350px] shadow-[0_0_40px_rgba(6,182,212,0.15)] flex flex-col overflow-hidden transition-all duration-500 origin-bottom-left ${isOpen ? 'opacity-100 scale-100 mb-4 h-[550px]' : 'opacity-0 scale-50 h-0 mb-0 pointer-events-none'}`}>
        ...
        <div className="bg-gradient-to-r from-cyan-500/10 to-purple-600/10 border-b border-white/5 p-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-[18px] tracking-widest text-white">NOVA AI</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
          {messages.map((m, i) => (
            /* Font impostato a minimo 18px per i messaggi */
            <div key={i} className={`p-3 text-[18px] rounded-2xl max-w-[85%] ${m.sender === 'user' ? 'bg-gradient-to-br from-cyan-500 to-blue-600 text-white self-end rounded-br-sm ml-auto font-medium shadow-lg' : 'bg-white/5 text-white/90 border border-white/5 self-start rounded-bl-sm leading-relaxed'}`} dangerouslySetInnerHTML={{ __html: m.text.replace(/\n/g, '<br>') }} />
          ))}
          {isLoading && (
            <div className="bg-white/5 text-cyan-400 border border-white/5 p-3 rounded-2xl rounded-bl-sm self-start max-w-[85%] text-[18px] flex gap-1">
        ...
        <div className="p-3 flex flex-wrap gap-2 bg-black/40 border-t border-white/5">
          {suggestions.map((text) => (
            /* Font a 18px per i suggerimenti di testo */
            <button key={text} onClick={() => sendMessage(text)} className="text-[18px] font-bold text-cyan-100 bg-cyan-500/10 hover:bg-cyan-500/30 border border-cyan-500/20 px-3 py-1.5 rounded-full transition-all">
              {text}
            </button>
          ))}
        </div>
        ...
        <div className="p-3 border-t border-white/5 bg-black/60 flex gap-2">
          {/* Font impostato a 18px per l'area di inserimento testo */}
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} placeholder="Scrivi a Nova..." className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-[18px] text-white outline-none focus:border-cyan-500/50 transition-colors" />
      ...
      {/* Tasto di attivazione: rettangolo con angoli smussati (rounded-2xl) e alternanza sequenziale dei loghi */}
      <button 
